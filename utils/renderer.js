...
/**
 * 绘制模式玩法提示（浅白大字，3次计数后渐隐消失）
 * @param {CanvasRenderingContext2D} ctx
 * @param {string} text - 提示文字
 * @param {number} count - 当前模式累计计数
 * @param {number} cx - 水平居中 x
 * @param {number} cy - 垂直位置 y
 */
export function drawModeHint(ctx, text, count, cx, cy, flashTime = 0) {
    if (count < 3) {
        ctx.save()
        let alpha = Math.max(0, 0.4 * (1 - count / 3))
        ...
        if (flashTime > 0) {
            ...
            if (elapsed < 300) {
                ...
                alpha = Math.min(1, alpha + flash * 0.55)
            }
        }
